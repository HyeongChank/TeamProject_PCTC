...
def make_model(route_data):
    base_data = route_data
    df = pd.read_csv(base_data, encoding='euc-kr')
    df = df[['trminlCode', 'landngComptQy', 'shipngComptQy','congestion']]
    data = df.dropna()
    ...
    def objective(trial):
        ...
        #     model_pkl = pickle.dump(model,f)
        # # requests.post("http://localhost:8080/api/model", data=model_pkl)   
        with open('model.pkl', 'wb') as f:
            model_pkl = pickle.dump(model,f)
...
if __name__=='__main__':
    route_data = "D:/김형찬/ai-algorism/data/congestion_data.csv"
    make_model(route_data)
