...
class Truck:
    def __init__(self, env, name, arrival_time, in_trucks_completed, out_trucks_completed, in_out_trucks_completed, arrival_truck):
        self.env = env
        self.name = name
        ...
        self.arrival_truck = arrival_truck
        #분포에 따라 customer 도착
    ...
    def truck_generate(self):
        ...
        arrival_truck.append(self.name)
...
# 트럭 대수
for i in range(200):
    Truck(env, i+1, i*arrival_interval, in_trucks_completed, out_trucks_completed, in_out_trucks_completed, arrival_truck)
print(waiting_times)
...
def init():
    ax.set_xlim(0, 240)  # 시뮬레이션 시간에 맞게 x축 범위 설정
    ax.set_ylim(0, 200)  # 트럭 대수에 맞게 y축 범위 설정
    return line,

def update(frame):
    time_interval = arrival_interval  # 트럭 도착 주기
    env.run(until=(frame+1) * time_interval)  # 프레임에 해당하는 시간까지 시뮬레이션 실행
    print(len(arrival_truck))
    print(arrival_truck)
    # 시뮬레이션 결과를 이용한 데이터 업데이트
    x_data.append(frame)
    y_data.append(len(arrival_truck))
